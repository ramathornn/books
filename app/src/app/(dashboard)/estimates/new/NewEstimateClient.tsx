...
export default function NewEstimatePage({ company }: { company: CompanyInfo }) {
  ...
  const [notes, setNotes] = useState(
    `Payment Methods:\n- E-Transfer: ${company.email}\n- Wire Transfer: Contact for details`
  )
  const [terms, setTerms] = useState(
    'This estimate is valid for 30 days from the date of issue. Upon acceptance, work will be scheduled and an invoice will be issued.'
  )
  ...
  // Load next estimate number
  useEffect(() => {
    fetch('/api/estimates/next-number')
      .then((res) => res.json())
      .then((data) => setEstimateNumber(data.nextNumber || ''))
      .catch(() => setEstimateNumber('0000175'))
  }, [])
  ...
  useEffect(() => {
    fetch('/api/items?limit=100')
      .then((res) => res.json())
      .then((data) => setCatalogItems(data.data || data || []))
      .catch(() => {})
  }, [])
  ...
  function updateLine(id: string, field: keyof LineItem, value: string | number | string[]) {
    setLineItems(
      lineItems.map((item) => (item.id === id ? { ...item, [field]: value } : item))
    )
  }

  function toggleGst(id: string) {
    setLineItems(
      lineItems.map((item) => {
  ...
  const handleCancel = useCallback(() => {
    if (isDirty) {
      pendingNavRef.current = '/estimates'
  ...
  const handleSave = useCallback(
    ...
    [clientId, estimateDate, currency, notes, lineItems, router]
  )
        ...
        <div className="flex-1">
          <div className="bg-white rounded-lg border border-gray-200 shadow-md p-8">
            ...
            <div className="flex justify-between mb-8">
              ...
              {/* Estimate meta fields - NO Due Date for estimates */}
              <div className="w-80">
                <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <label className="text-sm text-gray-500 self-center">
            ...
            <table className="w-full mb-4">
              <thead>
                <tr className="border-b-2 border-gray-200">
                  <th className="text-left py-1 text-xs font-medium text-gray-500 uppercase tracking-wider">
                    Description
                  </th>
                  <th className="text-right py-1 text-xs font-medium text-gray-500 uppercase tracking-wider w-28">
                    Rate
                  </th>
                  <th className="text-right py-1 text-xs font-medium text-gray-500 uppercase tracking-wider w-16">
                    Qty
                  </th>
                  <th className="text-right py-1 text-xs font-medium text-gray-500 uppercase tracking-wider w-28">
                    Line Total
                ...
                {lineItems.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-gray-100">
                      ...
                      <td className="py-1">
                        <div>
                          <input
                            ...
                            value={item.rate || ''}
                            onChange={(e) =>
                              updateLine(item.id, 'rate', parseFloat(e.target.value) || 0)
                            }
                            placeholder="0.00"
                            step="0.01"
                      ...
                      <td className="py-1 text-right text-sm text-gray-900 pr-2">
            ...
            <div className="mt-8 pt-6 border-t border-gray-200">
              ...
              <AutoTextarea
                ...
                minRows={4}
                className="w-full px-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-[#2FA84F] focus:border-[#2FA84F] resize-none"
              />
            </div>
