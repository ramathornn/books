...
export default function EditInvoicePage({
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([invoice, clientRes, itemsRes]: [InvoiceData, { data?: ClientData[] }, { data?: CatalogItem[] }]) => {
        ...
        setDateIssued(invoice.dateIssued ? invoice.dateIssued.split('T')[0] : '')
        setDateDue(invoice.dateDue ? invoice.dateDue.split('T')[0] : '')
        setInvoiceNumber(invoice.invoiceNumber || '')
        setCurrency(invoice.currency || 'CAD')
        setNotes(invoice.notes || '')
        setTerms(invoice.terms || '')
        setReference(invoice.reference || '')
        setOnlinePaymentsEnabled(Boolean(invoice.onlinePaymentsEnabled))
        const amtPaid = typeof invoice.amountPaid === 'string' ? parseFloat(invoice.amountPaid) : (invoice.amountPaid || 0)
        setExistingAmountPaid(amtPaid)
        const discountVal = typeof invoice.discount === 'string' ? parseFloat(invoice.discount) : invoice.discount
  ...
  }, [clientId, notes, terms, lineItems, dateIssued, dateDue, currency, discount, onlinePaymentsEnabled, loading])
  ...
  const amountDue = total - existingAmountPaid
  ...
  function toggleGst(lineId: string) {
    setLineItems(
      lineItems.map((item) => {
        if (item.id !== lineId) return item
        const hasGst = item.taxCodes.some((tc) => tc.startsWith('GST'))
        return {
          ...item,
          taxCodes: hasGst
            ? item.taxCodes.filter((tc) => !tc.startsWith('GST'))
            : [...item.taxCodes, 'GST:5'],
        }
      })
    )
  ...
  const handleCancel = useCallback(() => {
    if (isDirty) {
      pendingNavRef.current = `/invoices/${id}`
      ...
      if (!res.ok) {
        ...
        const details = data.details
          ? Object.entries(data.details as Record<string, string[]>)
              ...
              .join(' · ')
          : ''
        throw new Error(details ? `${data.error || 'Validation failed'} — ${details}` : data.error || 'Failed to update invoice')
      }
  ...
  }, [clientId, dateIssued, dateDue, currency, notes, terms, reference, lineItems, discount, onlinePaymentsEnabled, id, router])
  ...
  return (
    <div>
      {/* Top bar: Cancel | Save | Send To... */}
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-[40px] leading-[48px] font-medium text-[#001B40]">Edit Invoice</h1>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={saving}
            className="px-5 py-2 bg-[#2FA84F] hover:bg-[#268f3e] text-white text-sm font-medium rounded-md transition-colors disabled:opacity-50 inline-flex items-center gap-1"
          >
            Send To...
      ...
      <div className="flex gap-6 items-start">
        <div className="flex-1 min-w-0 max-w-[820px]">
          <div className="bg-white rounded-sm shadow-md px-12 py-12 lg:px-16 lg:py-16">
            {/* Company header: logo placeholder (top-left) + business info (top-right) */}
            ...
            <div className="flex justify-end">
              <div className="w-72">
                ...
                <div className="flex justify-between py-2 text-sm">
                  ...
                  <span className="text-gray-900">{fmtPlain(existingAmountPaid)}</span>
                </div>
